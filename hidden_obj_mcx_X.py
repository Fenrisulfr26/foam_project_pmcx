# -*- coding: utf-8 -*-
"""
Created on Tue Sep 23 22:35:39 2025
...
np.save('sim_MCX_obj_X_3x3.npy',sim_mea) # file name first and then variable name
# sim_mea_save = sim_mea
#%% show the intensity results
# sim_mea = np.load('sim_mea_obj_L.npy')

mea = np.load(os.path.join(data_fold,r'X_gain0.7_5x5cm_31x31points_pol50deg_expo0.5sec_binWidth15ps_binNum2000_061224.npy'));

mea_down = np.zeros((31, 31, 101))
for i in range(31):
    for j in range(31):
...
# IRF_down = resize(IRF[:,np.round(121*100/15)], (1, 121), interpolation=cv2.INTER_NEAREST_EXACT)
IRF_down= np.squeeze(IRF_down)

cfg['tend'] = sim_len*1e-10

sim_mea_3x3 = np.zeros((3,3,sim_len));
...
for i, pos_y in tqdm(enumerate(src_y_positions_3x3)):
    for j, pos_x in tqdm(enumerate(src_x_positions_3x3)):
        ...
        sim_mea_3x3[i,j,:] = (dref_bd_rot*sens_pad[:,:,None]).sum((0,1)) # apply the sensitivity map
            
sim_mea_3x3 = np.apply_along_axis(lambda m: fftconvolve(m, IRF_down, mode='full'), axis=2, arr=sim_mea_3x3)[:,:,:sim_len]

sim_mea = sim_mea/sim_mea.max()
    
mea = np.load(os.path.join(data_fold,r'X_gain0.7_5x5cm_31x31points_pol50deg_expo0.5sec_binWidth15ps_binNum2000_061224.npy'));

mea_down3x3 = np.zeros((3, 3, sim_len))

for i, index_i in enumerate([0, 15, 30]):
    for j,index_j in enumerate([0, 15, 30]):
        mea_down3x3[i,j,:] = np.squeeze(resize(mea[index_i,index_j,:int(np.round(sim_len*100/15))], (1, sim_len), 
                                             interpolation=cv2.INTER_NEAREST_EXACT))
