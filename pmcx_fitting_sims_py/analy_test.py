...
use analytical data to test if the optimization works good
...
def fitfn(cfg, mu_a, mu_s):
    ...
    return conv_simp[:,:,:len(IRF)].flatten() 
...
p, pcov = curve_fit(fitfn, cfg, fitfn(cfg, mu_a, mu_s) , p0=[0.0025883,1.68196508], 
...
grd_tru = np.reshape(fitfn(cfg,0.01,1),[3,3,101])
